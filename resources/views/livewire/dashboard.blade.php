...
// PERBAIKAN: Gunakan fungsi layout() dan title()
...
state([
    ...
    // Dosen Pembimbing (juga digunakan oleh Dosen Komisi)
...
mount(function () {
    ...
    if ($user->role === 'Mahasiswa' && $user->mahasiswa) {
        ...
        $this->activeKp = KerjaPraktek::with(['seminar', 'konsultasis'])->where('mahasiswa_id', $mahasiswaId)->latest()->first();
        $this->suratDisetujui = SuratPengantar::where('mahasiswa_id', $mahasiswaId)->where('status_surat_pengantar', 'Disetujui')->exists();
    ...
    // Logika untuk semua peran DOSEN (Komisi & Pembimbing)
    if ($user->dosen) {
                        ...
                        <div class="space-y-2">
                            @foreach($steps as $number => $step)
                                ...
                                <a href="{{ route($step['href']) }}" class="block p-4 rounded-lg transition-colors
                                   {{ $isActive ? 'bg-primary-50 dark:bg-primary-900/20 border border-primary-300 dark:border-primary-700' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800' }}">
            ...
            @elseif (in_array(auth()->user()->role, ['Dosen Pembimbing', 'Dosen Komisi']))
                <div class="space-y-8">
                    ...
                    @if(auth()->user()->role === 'Dosen Komisi')
                        <div>
                            <flux:heading size="lg" level="2">Tugas Sebagai Dosen Komisi</flux:heading>
                    ...
                    <div>
                        <flux:heading size="lg" level="2" class="{{ auth()->user()->role === 'Dosen Komisi' ? 'mt-8' : '' }}">Tugas Sebagai Dosen Pembimbing</flux:heading>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-4">
